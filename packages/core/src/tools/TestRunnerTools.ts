...
export class TestRunner {
     ...
     */
    async detectTestFramework(): Promise<'jest' | 'vitest' | 'mocha' | 'npm-test' | 'unknown'> {
        try {
     ...
     */
    async runForModifiedFile(filePath: string): Promise<TestResult> {
 ...
 */
export const TestRunnerTools = (projectRoot: string = process.cwd()) => {
    ...
    return [
        {
            ...
            handler: async (args: { testFile?: string, all?: boolean }) => {
        ...
        },
        {
            ...
            handler: async (args: { sourceFile: string }) => {
