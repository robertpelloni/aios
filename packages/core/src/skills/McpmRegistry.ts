...
import { MarketplaceService, MarketplaceItem } from "./MarketplaceService.js";
...
export class McpmRegistry {
    private marketplace = new MarketplaceService();
    private registry: MCPPackage[] = [
        {
            name: "sqlite",
        ...
        return [
            ...
            },
            {
                ...
                handler: async (args: { query: string }) => {
                    ...
                    const localMatches = this.registry.filter(p =>
                        p.name.includes(q) ||
                        p.description.toLowerCase().includes(q)
                    );

                    const marketIndex = await this.marketplace.getIndex();
                    const marketMatches = marketIndex.filter(p =>
            ...
            },
            {
                ...
                handler: async (args: { name: string, alias?: string, manualCommand?: string }) => {
                    let pkg: MCPPackage | undefined = this.registry.find(p => p.name === args.name);
                    ...
                    if (!pkg) return { content: [{ type: "text", text: `Error: Package '${args.name}' not found.` }] };
