...
export class CodeExecutionManager {
    ...
    private async executeIsolated(code: string, toolCallback: (name: string, args: any) => Promise<any>, sessionId?: string): Promise<string> {
        try {
            ...
            const script = await session.context.evalClosure(wrappedCode, [], { result: { promise: true, copy: true } });
            // Wait, evalClosure isn't right for running arbitrary string code.
            // We compileScript.

            const scriptObj = await session.context.isolate.compileScript(wrappedCode);
