        ...
        <Card className="hover:shadow-lg transition-shadow">
            <CardContent>
                <div className="flex items-center justify-between">
                    ...
                    <Badge variant={color === 'green' ? 'default' : color === 'yellow' ? 'secondary' : color === 'red' ? 'destructive' : 'default'}>
...
export function MCPRouterStats({ registryStats, sessionStats, healthStatus, loading = false }: MCPRouterStatsProps) {
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <StatusCard
                title="System Health"
                value={healthStatus?.status === 'healthy' ? 'OK' : healthStatus?.status?.toUpperCase()}
