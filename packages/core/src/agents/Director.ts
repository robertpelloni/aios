...
export class Director {
    ...
    private autoAccepterInterval: NodeJS.Timeout | null = null;
    ...
    private async detectConversationState(): Promise<'AI_WORKING' | 'NEEDS_SUBMIT' | 'IDLE' | 'UNKNOWN'> {
        try {
            ...
            // @ts-ignore
            if (result?.content?.[0]?.text?.includes('generating')) return 'AI_WORKING';
            // @ts-ignore
            if (result?.content?.[0]?.text?.includes('ready')) return 'NEEDS_SUBMIT';
        } catch {
        ...
        if (idleMs < 5000) return 'AI_WORKING'; // Recently active, assume working
        if (idleMs < 30000) return 'NEEDS_SUBMIT'; // 5-30s idle, try to submit
        return 'IDLE'; // > 30s idle
    }

    private async respondToState(state: 'AI_WORKING' | 'NEEDS_SUBMIT' | 'IDLE' | 'UNKNOWN') {
        switch (state) {
            case 'AI_WORKING':
    ...
    private async sendGemmaEncouragement() {
        try {
    ...
    private startAutoAccepter() {
        if (this.autoAccepterInterval) return;
