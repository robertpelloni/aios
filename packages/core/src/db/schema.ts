...
export type ToolStatus = 'active' | 'disabled' | 'hidden';
...
export interface Endpoint {
    id: string;
    name: string;
    path: string; // e.g., "/api/mcp/coding"
    namespaceId: string;
    apiKeyId?: string;
    enabled: boolean;
    createdAt: number;
...
export interface NamespaceServerMapping {
    id: string;
    namespaceId: string;
    mcpServerId: string;
...
export interface NamespaceToolMapping {
    id: string;
    namespaceId: string;
    toolId: string;
    status: ToolStatus;
    aliasName?: string; // Override tool name in this namespace
    createdAt: number;
    updatedAt: number;
}
...
export interface PolicyRule {
    action: PolicyAction;
    pattern: string; // Glob pattern for tool/server names
    conditions?: {
...
export function fromDbRow<T>(row: Record<string, unknown>, jsonFields: string[] = []): T {
